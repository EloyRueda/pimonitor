...
function updateDashboard() {
    fetch('/data')
        ...
        .then(data => {
            ...
            // 3. Actualizar velocidades de Red
            const elDescarga = document.getElementById('red-descarga');
            const elSubida = document.getElementById('red-subida');
            if (elDescarga && data.red_descarga !== undefined) elDescarga.innerText = formatearVelocidad(data.red_descarga);
            if (elSubida && data.red_subida !== undefined) elSubida.innerText = formatearVelocidad(data.red_subida);
        })
        .catch(err => console.error("Error al obtener datos:", err));
}

function formatearVelocidad(kbps) {
    if (typeof kbps !== 'number') return "0.00 KB/s";
    if (kbps >= 1024) {
...
function actualizarDispositivosRed() {
        ...
        data.hosts.forEach(host => {
            tbody.innerHTML += `
                <tr style="border-bottom: 1px solid #2d3748; height: 45px;">
                    ...
                    <td style="color: #63b3ed; font-size: 0.9em;">${host.vendor}</td> 
                    <td style="font-family: monospace; color: #cbd5e1; letter-spacing: 0.5px;">${host.mac}</td>
    ...
    .catch(error => {
        ...
        const tbody = document.getElementById('lista-hosts');
        if (tbody) tbody.innerHTML = `<tr><td colspan="4" style="padding: 15px 0; text-align: center; color: #e53e3e;">Error al compilar la tabla ARP del sistema</td></tr>`;
    });
}
